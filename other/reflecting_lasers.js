 ...
 * reflecting_laser.js
 *
 * Rocket Fuel's HackerRank coding challenge.
 * 3 hours. Use object-oriented design to write
 * readable, maintainable, modular code.
 *
 * A field is given. The field has prisms (^, v, <, >) which
 * redirect a laser in the direction they're pointing,
 * regardless of entry direction. (Down is the lowercase
 * letter v as in victor)
 *
 ...
 * If an infinite loop exists, detect it immediately and print
 * -1. Going through a loop twice before printing -1 is unacceptable.
 */
...
function lasers(input) {
  ...
  while(1) {
     ...
     * the set. This way, no mirror/prism does the same thing twice. The
     * loop is detected as soon as there is a single repeat.
     */
    if(thisChar !== '-') {
