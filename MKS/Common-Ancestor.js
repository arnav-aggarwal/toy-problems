...
Tree.prototype.getClosestCommonAncestor = function(node1, node2){
  ...
  while(node1 = node1.parent) {
    if(node1.isDescendant(node2)) return node1;
  }
  
  return null;
}

Tree.prototype.getAncestorPath = function(current){
  if(!this.isDescendant(current)) return null;
  const ancestry = [current];
  while(current = current.parent) ancestry.push(current);
